...
Connection-liveness mechanisms, like a keepalive ping, also don't count, even though they can involve an actual request and response. A ping only confirms that a connection is still alive; it doesn't carry or request any entity state. IoT class describes how state data is retrieved, not how a persistent connection is kept alive, and almost every push-based protocol needs some form of keepalive, so counting it as a secondary class would make it apply almost everywhere and stop being meaningful.
